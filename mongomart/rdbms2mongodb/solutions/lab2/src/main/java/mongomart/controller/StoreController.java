...
public class StoreController {
	...
	public StoreController(Configuration cfg, Connection connection, MongoDatabase mongoMartDatabase) {
		...
		mongomart.dao.mongodb.ReviewDao mongoReviewDao = new mongomart.dao.mongodb.ReviewDao(mongoMartDatabase);
		...
		get("/item", (request, response) -> {
			...
			HashMap<String, Object> attributes = buildItemResponse(Utils.getIntFromString(itemid), itemDao, mongoItemDao, reviewDao, mongoReviewDao);
	 ...
	 * @return
	 */
	private HashMap<String, Object> buildItemResponse(int itemid, ItemDao itemDao, mongomart.dao.mongodb.ItemDao mongoItemDao, ReviewDao reviewDao, mongomart.dao.mongodb.ReviewDao mongoRevDao) {
		List<Item> related_items = itemDao.getItems("0");
		boolean newItemToMongo = false;
		Item item = mongoItemDao.getItem(itemid);

		// if MongoDB does not have the item, looks find it in the rdbms
		if(item.getId() < 1 ){
		...
		if (newItemToMongo && item.getId() > 0){
			mongoItemDao.addItem(item);
		}
