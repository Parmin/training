...
public class ItemDao {
	...
	public void addItem(Item item){
		Document doc = ItemDao.itemToDocument(item);
	...
	public static Document itemToDocument(Item item){
		...
		itemDoc.append("_id", item.getId());
		itemDoc.append("title", item.getTitle());
		itemDoc.append("description", item.getDescription());
		itemDoc.append("category", item.getCategory());
		itemDoc.append("price", item.getPrice());
		itemDoc.append("stars", item.getStars());
		itemDoc.append("img_url", item.getImg_url());
		itemDoc.append("slogan", item.getSlogan());

		List<Document> reviews = new ArrayList<Document>();
		for(Review review : item.getReviews()){
			reviews.add(reviewToDoc(review));
		}
		itemDoc.append("reviews", reviews);
